...
// const stuQuery = require('./stuquery.js');
// let module = require('./starsData.mjs');
// let array = module.array;
// import array from './starsData.mjs';
...
function VirtualSky(input){
	...
	this.constellation = { lines: true, boundaries: false, labels: true };	// Display constellations
	...
	this.showstarlabels = true;		// Display names for named stars
	...
	this.showdate = false;				// Display the current date
	this.showposition = false;			// Display the longitude/latitude
	this.scalestars = 1.5;				// A scale factor by which to increase the star sizes
	this.ground = false;
	...
	this.keyboard = false;				// Allow keyboard controls
	...
	this.lookup.star = [];
	for(let i = 0; i < this.stars.length; i++) this.lookup.star.push({'ra':this.stars[i][2],'dec':this.stars[i][3],'label':this.stars[i][0],'mag':this.stars[i][1]});
	...
	this.fullsky = true;         // Are we showing the entire sky?
...
VirtualSky.prototype.panTo = function(uniqueId){
	let ra, dec, s

	for (let i = 0; i < this.pointers.length; i++) {
		if (this.pointers[i].uniqueId === uniqueId) {
			ra = this.pointers[i].ra;
			dec = this.pointers[i].dec;
		}
	}
	if(!s) s = 1000;
...
VirtualSky.prototype.addPointer = function(input){
	...
	if(matched < 0){
		...
		// 	style = p.style || "width:500px;height:128px;";
		// 	url = p.url || "http://www.reddit.com";
...
VirtualSky.prototype.mapStarCards = function(cards){
	...
	cards.map((card) => S("div#cards").prepend(`<div id="cardsContainer">${S("input#name")[0].value}<img src="./svg-5.svg" "alt="star" id="card"/>${this.pointers[this.pointers.length - 1].uniqueId}</div>`));
}
